...
The project uses a single Django app (preview) with the domain logic deliberately isolated from the view layer:

hris_project/settings.py - Minimal settings (uses in-memory SQLite to satisfy test runner requirements).

preview/analyzer.py - Pure Python domain layer handling all parsing, validation, and graph logic.

preview/views.py - Thin view handling the file upload, decoding, and passing context to the template.

preview/tests.py - Automated test suite verifying the domain logic in analyzer.py.

preview/templates/preview/index.html - The single-page frontend.
